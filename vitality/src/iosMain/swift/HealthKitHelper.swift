...
@objc public class WatchConnectivityManager: NSObject {
    private let session: WCSession
    private weak var delegate: WatchConnectivityDelegate?
    private let healthStore = HKHealthStore()
    
    // Message queues for when watch is not reachable
    private var pendingMessages: [(message: [String: Any], replyHandler: ((Data) -> Void)?)] = []
    private var pendingTransfers: [WCSessionFileTransfer] = []
    
    // Current workout sessions (only store session IDs since HKWorkoutSession requires iOS 17+)
    private var activeWorkoutSessionIds: Set<String> = []
    private var workoutBuilders: [String: HKWorkoutBuilder] = [:]
    ...
    @objc public func setDelegate(_ delegate: WatchConnectivityDelegate) {
        self.delegate = delegate
    }
    
    @objc public func getConnectivityStatus() -> ConnectivityStatus {
    ...
    @objc public func sendMessage(_ message: [String: Any], completion: @escaping (Bool, Error?) -> Void) {
        guard session.isReachable else {
            // Queue message for later delivery
            pendingMessages.append((message, nil))
            completion(false, WatchConnectivityError.watchNotReachable)
            return
        }
    ...
    @objc public func sendMessageWithReply(_ message: [String: Any], completion: @escaping (Data?, Error?) -> Void) {
        guard session.isReachable else {
            completion(nil, WatchConnectivityError.watchNotReachable)
            return
        }
        
        session.sendMessage(message, replyHandler: { reply in
            // Convert reply dictionary to JSON data
            do {
                let data = try JSONSerialization.data(withJSONObject: reply, options: [])
                completion(data, nil)
            } catch {
                completion(nil, error)
            }
    ...
    @objc public func updateApplicationContext(_ context: [String: Any], completion: @escaping (Bool, Error?) -> Void) {
        do {
    ...
    @objc public func transferUserInfo(_ userInfo: [String: Any]) -> WCSessionUserInfoTransfer {
        return session.transferUserInfo(userInfo)
    }
    
    @objc public func transferFile(_ fileURL: URL, metadata: [String: Any]?) -> WCSessionFileTransfer {
        let transfer = session.transferFile(fileURL, metadata: metadata)
        pendingTransfers.append(transfer)
        return transfer
    }
    
    @objc public func applicationContext() -> [String: Any] {
        return session.applicationContext
    }
    
    @objc public func receivedApplicationContext() -> [String: Any] {
        return session.receivedApplicationContext
    }
    
    @objc public func processPendingMessages() {
        guard session.isReachable else { return }
        
        let messages = pendingMessages
        pendingMessages.removeAll()
        
        for (message, replyHandler) in messages {
            if let replyHandler = replyHandler {
                sendMessageWithReply(message) { data, error in
                    if let data = data {
                        replyHandler(data)
                    }
                }
    ...
    @objc public func queueMessage(_ message: [String: Any]) {
        pendingMessages.append((message, nil))
    }
    ...
    private func processIncomingMessage(_ message: [String: Any]) {
        // Convert message to format expected by Kotlin
        let appMessage: [String: Any] = [
        ...
        delegate?.didReceiveMessage(appMessage)
    }
    ...
    private func sendPendingMessages() {
        ...
        for (message, replyHandler) in messages {
            if let replyHandler = replyHandler {
                sendMessageWithReply(message) { data, _ in
                    if let data = data {
                        replyHandler(data)
                    }
                }
...
extension WatchConnectivityManager: WCSessionDelegate {
    public func session(_ session: WCSession, activationDidCompleteWith activationState: WCSessionActivationState, error: Error?) {
        if activationState == .activated {
            delegate?.watchConnectivityActivated()
    ...
    public func sessionDidBecomeInactive(_ session: WCSession) {
        delegate?.watchConnectivityBecameInactive()
    }
    ...
    public func sessionReachabilityDidChange(_ session: WCSession) {
        delegate?.watchReachabilityChanged(isReachable: session.isReachable)
    ...
    public func session(_ session: WCSession, didReceiveMessage message: [String: Any], replyHandler: @escaping ([String: Any]) -> Void) {
        processReceivedMessage(message, replyHandler: replyHandler)
    }
    ...
    public func session(_ session: WCSession, didFinish fileTransfer: WCSessionFileTransfer, error: Error?) {
        if let index = pendingTransfers.firstIndex(of: fileTransfer) {
            pendingTransfers.remove(at: index)
        }
        
        delegate?.fileTransferCompleted(fileURL: fileTransfer.file.fileURL, error: error)
    }
    
    private func processReceivedMessage(_ message: [String: Any], replyHandler: (([String: Any]) -> Void)? = nil) {
        // Process as generic app message
        processIncomingMessage(message)
        
        // If reply is expected, let the app handle it
        if let replyHandler = replyHandler {
            delegate?.didReceiveMessageWithReplyHandler(message) { reply in
                replyHandler(reply)
            }
        }
    }
